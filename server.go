...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// first, try with verb "get:/v1/resource"
	...
	if exists && respInfo.ContentType != "" {
		w.Header().Set(httpHeaderContentType, respInfo.ContentType)
		switch respInfo.ContentType {
		case httpHeaderTextPlain:
			fileExt = ".txt"
		case httpHeaderApplicationTimestampQuery:
			fileExt = ".tsq"
		case httpHeaderApplicationTimestampReply:
			fileExt = ".tsr"
		default:
			fileExt = ".json"
		}
	} else {
		w.Header().Set(httpHeaderContentType, httpHeaderApplicationJSON)
	}
...
const (
	...
	httpHeaderApplicationJSON           = "application/json"
	httpHeaderApplicationJoseJSON       = "application/jose+json"
	httpHeaderApplicationGRPC           = "application/grpc"
	httpHeaderApplicationTimestampQuery = "application/timestamp-query"
	httpHeaderApplicationTimestampReply = "application/timestamp-reply"
	httpHeaderAuthorization             = "Authorization"
	httpHeaderContentEncoding           = "Content-Encoding"
	httpHeaderContentLength             = "Content-Length"
	httpHeaderContentType               = "Content-Type"
	httpHeaderTextPlain                 = "text/plain"
)
